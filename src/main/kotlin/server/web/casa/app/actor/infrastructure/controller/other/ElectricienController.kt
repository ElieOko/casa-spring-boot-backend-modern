...
import server.web.casa.app.actor.application.service.other.ElectricienService
...
const val ROUTE_ACTOR_ELECTRICIEN = ActorRoute.ELECTRICIEN

@Tag(name = "ELECTRICIEN", description = "Gestion des ELECTRICIEN")
@RestController
@RequestMapping(ROUTE_ACTOR_ELECTRICIEN)
@Profile(Mode.DEV)
class ElectricienController(
    private val service : ElectricienService,
    ...
    suspend fun createElectricien(
        ...
        if (request.user.typeAccountId != 7L){
...
//        val typeCard = typeCardService.findByIdTypeCard(request.locataire.typeCardId)
    ...
    suspend fun getAllElectricien(): ResponseEntity<Map<String, List<Electricien>>> {
        val data = service.findAll()
        val response = mapOf("electriciens" to data)
        return ResponseEntity.ok().body(response)
    }

    @Operation(summary = "Modification Electricien")
    @PutMapping("/{id}")
    suspend fun updateElectricien(
        @PathVariable("id") id : Long,
        @RequestBody @Valid data: Electricien
    ) : ResponseEntity<Electricien> {
