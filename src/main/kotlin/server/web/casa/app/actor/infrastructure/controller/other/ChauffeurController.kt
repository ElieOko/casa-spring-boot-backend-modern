...
import server.web.casa.app.actor.application.service.other.ChauffeurService
import server.web.casa.app.actor.domain.model.*
import server.web.casa.app.actor.domain.model.join.other.ChauffeurUser
...
const val ROUTE_ACTOR_CHAUFFEUR = ActorRoute.CHAUFFEUR

@Tag(name = "CHAUFFEUR", description = "Gestion des CHAUFFEUR")
@RestController
@RequestMapping(ROUTE_ACTOR_CHAUFFEUR)
@Profile(Mode.DEV)
class ChauffeurController(
    private val service : ChauffeurService,
    ...
    suspend fun createChauffeur(
        @Valid @RequestBody request: ChauffeurUser
    ): ResponseEntity<Map<String, Any?>> {
        val typeAccount = typeAccountService.findByIdTypeAccount(request.user.typeAccountId)
        if (request.user.typeAccountId != 15L){
...
//        val typeCard = typeCardService.findByIdTypeCard(request.locataire.typeCardId)
        if (typeAccount != null) {
            val userSystem = User(
                ...
                username = "@"+toPascalCase("${request.chauffeur.firstName} ${request.chauffeur.lastName}")
            )
            val userCreated = authService.register(userSystem)
            val data = Chauffeur(
                firstName = request.chauffeur.firstName,
                lastName = request.chauffeur.lastName,
                fullName = "${request.chauffeur.firstName} ${request.chauffeur.lastName}",
                address = request.chauffeur.address,
                images = request.chauffeur.images,
                cardFront = request.chauffeur.cardFront,
                cardBack = request.chauffeur.cardBack,
                user = userCreated.first,
                typeCard = null,
                numberCard = request.chauffeur.numberCard
            )
    ...
    suspend fun getAllChauffeur(): ResponseEntity<Map<String, List<Chauffeur>>> {
        val data = service.findAll()
        val response = mapOf("chauffeurs" to data)
        return ResponseEntity.ok().body(response)
    }

    @Operation(summary = "Modification Chauffeur")
    @PutMapping("/{id}")
    suspend fun updateChauffeur(
        @PathVariable("id") id : Long,
        @RequestBody @Valid data: Chauffeur
    ) : ResponseEntity<Chauffeur> {
