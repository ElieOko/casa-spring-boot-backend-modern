...
import server.web.casa.app.ecosystem.application.service.realisation.RealisationMaconServiceAction
import server.web.casa.app.ecosystem.application.service.task.AjusteurServiceAction
import server.web.casa.app.ecosystem.application.service.task.MaconServiceAction
...
import server.web.casa.app.ecosystem.domain.request.MaconServiceRequest
...
import server.web.casa.app.ecosystem.infrastructure.persistence.repository.service.ServiceMaconRepository
...
@Tag(name = "Maçon Service", description = "Gestion des services Maçon")
@RestController
@RequestMapping(Service.MACON)
class MaconServiceController(
    private val userService: UserService,
    private val deviseService: DeviseService,
    private val realisationImage : RealisationMaconServiceAction,
    private val serviceAction : MaconServiceAction,
    private val cityService: CityService,
    private val communeService: CommuneService,
    private val quartierService: QuartierService
) {
    @PostMapping(consumes = [MediaType.APPLICATION_JSON_VALUE])
    suspend fun createMaconService(
        @Valid @RequestBody request: MaconServiceRequest,
    ...
    @Operation(summary = "Voir les maçons service")
    @GetMapping(produces = [MediaType.APPLICATION_JSON_VALUE])
    suspend fun getAllMenusier(): ResponseEntity<Map<String, List<MaconTask>>> {
        val response = mapOf("macons" to serviceAction.getAllData())
