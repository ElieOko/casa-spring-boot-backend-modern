...
@Tag(name = "Plombier Service", description = "Gestion des services Plombier")
@RestController
@RequestMapping(Service.PLOMBIER)
class PlombierServiceController(
    private val userService: UserService,
    private val deviseService: DeviseService,
    private val realisationImage : RealisationPlombierServiceAction,
    private val serviceAction : PlombierServiceAction,
    private val cityService: CityService,
    private val communeService: CommuneService,
    private val quartierService: QuartierService
) {
    @PostMapping(consumes = [MediaType.APPLICATION_JSON_VALUE])
    suspend fun createPlombierService(
        @Valid @RequestBody request: PlombierServiceRequest,
        ...
        if (realisation.isNotEmpty()){
            val entity = ServicePlombierEntity(
    ...
    @Operation(summary = "Voir les plombiers service")
    @GetMapping(produces = [MediaType.APPLICATION_JSON_VALUE])
    suspend fun getAllPlombier(): ResponseEntity<Map<String, List<PlombierTask>>> {
        val response = mapOf("plombiers" to serviceAction.getAllData())
