...
@Service
class MaconServiceAction(
    private val repository : ServiceMaconRepository
) {
    fun create(data : MaconTask) = repository.save(data.toEntity()).toDomain()
    fun canCertified(id : Long): Pair<String, MaconTask> {
    ...
    fun getAllData(): List<MaconTask> {
        val data = repository.findAll().filter { it.isActive && it.isCertified }
        return data.map{it.toDomain()}
    }
    fun canUpdate(id : Long, data: MaconTask): Pair<String, MaconTask> {
