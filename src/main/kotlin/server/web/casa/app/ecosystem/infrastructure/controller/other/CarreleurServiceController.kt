...
@Tag(name = "Carreleur Service", description = "Gestion des services Carreleur")
@RestController
@RequestMapping(Service.CARRELEUR)
class CarreleurServiceController(
    private val userService: UserService,
    private val deviseService: DeviseService,
    private val serviceAction : CarreleurServiceAction,
    private val realisationImage : RealisationCarreleurServiceAction,
    private val cityService: CityService,
    private val communeService: CommuneService,
    private val quartierService: QuartierService
) {
    @PostMapping(consumes = [MediaType.APPLICATION_JSON_VALUE])
    suspend fun createCarreleurService(
        @Valid @RequestBody request: CarreleurServiceRequest,
        ...
        if (realisation.isNotEmpty()){
            val entity = ServiceCarreleurEntity(
    ...
    @Operation(summary = "Voir les Carreleur service")
    @GetMapping(produces = [MediaType.APPLICATION_JSON_VALUE])
    suspend fun getAllCarreleur(): ResponseEntity<Map<String, List<CarreleurTask>>> {
        val response = mapOf("carreleur" to serviceAction.getAllData())
