...
import server.web.casa.app.ecosystem.application.service.realisation.RealisationMenusierServiceAction
import server.web.casa.app.ecosystem.application.service.task.AjusteurServiceAction
import server.web.casa.app.ecosystem.application.service.task.MenusierServiceAction
...
import server.web.casa.app.ecosystem.domain.request.MenusierServiceRequest
...
@Tag(name = "Menusier Service", description = "Gestion des services Menusier")
@RestController
@RequestMapping(Service.MENUSIER)
class MenusierServiceController(
    private val userService: UserService,
    private val deviseService: DeviseService,
    private val realisationImage : RealisationMenusierServiceAction,
    private val serviceAction : MenusierServiceAction,
    private val cityService: CityService,
    private val communeService: CommuneService,
    private val quartierService: QuartierService
) {
    @PostMapping(consumes = [MediaType.APPLICATION_JSON_VALUE])
    suspend fun createMenusierService(
        @Valid @RequestBody request: MenusierServiceRequest,
        ...
        if (realisation.isNotEmpty()){
            val entity = ServiceMenusierEntity(
    ...
    @Operation(summary = "Voir les menusiers service")
    @GetMapping(produces = [MediaType.APPLICATION_JSON_VALUE])
    suspend fun getAllMenusier(): ResponseEntity<Map<String, List<MenusierTask>>> {
        val response = mapOf("menusiers" to serviceAction.getAllData())
