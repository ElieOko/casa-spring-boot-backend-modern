...
import server.web.casa.app.ecosystem.infrastructure.persistence.repository.realisation.MenusierRealisationRepository
import server.web.casa.utils.base64ToMultipartFile
import server.web.casa.utils.gcs.GcsService
import server.web.casa.utils.storage.FileSystemStorageService

@Service
class RealisationMenusierServiceAction(
    private val repository: MenusierRealisationRepository,
    private val gcsService: GcsService,
    private val storageService: FileSystemStorageService
) {
    private val log = LoggerFactory.getLogger(this::class.java)
    private val model = "menusier"
    suspend fun create(p : MenusierRealisation,server : String): MenusierRealisation {
...
//        log.info("public url local $fileUrl")
        p.path = imageUri!!
        val data = ServiceMenusierRealisationEntity(
