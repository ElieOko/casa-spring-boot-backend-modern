...
import server.web.casa.app.ecosystem.infrastructure.persistence.repository.realisation.CarreleurRealisationRepository
import server.web.casa.utils.base64ToMultipartFile
import server.web.casa.utils.gcs.GcsService
import server.web.casa.utils.storage.FileSystemStorageService

@Service
class RealisationCarreleurServiceAction(
    private val repository : CarreleurRealisationRepository,
    private val gcsService: GcsService,
    private val storageService: FileSystemStorageService
) {
    private val log = LoggerFactory.getLogger(this::class.java)
    private val model = "carreleur"
    suspend fun create(p : CarreleurRealisation,server : String): CarreleurRealisation {
...
//        log.info("public url local $fileUrl")
        p.path = imageUri!!
        val data = ServiceCarreleurRealisationEntity(
