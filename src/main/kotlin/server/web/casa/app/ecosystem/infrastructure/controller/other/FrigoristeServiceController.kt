...
import server.web.casa.app.ecosystem.application.service.realisation.RealisationFrigoristeServiceAction
import server.web.casa.app.ecosystem.application.service.task.AjusteurServiceAction
import server.web.casa.app.ecosystem.application.service.task.FrigoristeServiceAction
...
import server.web.casa.app.ecosystem.domain.request.FrigoristeServiceRequest
...
@Tag(name = "Frigoriste Service", description = "Gestion des services Frigoriste")
@RestController
@RequestMapping(Service.FRIGORISTE)
class FrigoristeServiceController(
    private val userService: UserService,
    private val deviseService: DeviseService,
    private val realisationImage : RealisationFrigoristeServiceAction,
    private val serviceAction : FrigoristeServiceAction,
    private val cityService: CityService,
    private val communeService: CommuneService,
    private val quartierService: QuartierService
) {
    @PostMapping(consumes = [MediaType.APPLICATION_JSON_VALUE])
    suspend fun createFrigoristeService(
        @Valid @RequestBody request: FrigoristeServiceRequest,
        ...
        if (realisation.isNotEmpty()){
            val entity = ServiceFrigoristeEntity(
    ...
    @Operation(summary = "Voir les Frigoristes service")
    @GetMapping(produces = [MediaType.APPLICATION_JSON_VALUE])
    suspend fun getAllFrigoriste(): ResponseEntity<Map<String, List<FrigoristeTask>>> {
        val response = mapOf("frigoristes" to serviceAction.getAllData())
