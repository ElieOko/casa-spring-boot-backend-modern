package server.web.casa.app.ecosystem.infrastructure.persistence.entity.architect
...
@Entity
@Table(name = "service_architect_realisations")
class ServiceArchitectRealisationEntity (
    @Id
    @Column("id")
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    val id : Long = 0,
    @ManyToOne
    @JoinColumn("service_architect_id")
    @JsonBackReference
    var service : ServiceArchitectEntity,
    @Column(name = "name")
    val name : String,
...
fun ServiceArchitectRealisationEntity.toDomain() = ArchitectRealisation(
    id = this.id,
