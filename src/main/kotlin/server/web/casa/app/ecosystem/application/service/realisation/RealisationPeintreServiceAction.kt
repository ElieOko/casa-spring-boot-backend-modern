...
import server.web.casa.app.ecosystem.infrastructure.persistence.repository.realisation.PeintreRealisationRepository
import server.web.casa.utils.base64ToMultipartFile
import server.web.casa.utils.gcs.GcsService
import server.web.casa.utils.storage.FileSystemStorageService

@Service
class RealisationPeintreServiceAction(
    private val repository: PeintreRealisationRepository,
    private val gcsService: GcsService,
    private val storageService: FileSystemStorageService
) {
    private val log = LoggerFactory.getLogger(this::class.java)
    private val model = "peintre"
    suspend fun create(p : PeintreRealisation,server : String): PeintreRealisation {
...
//        log.info("public url local $fileUrl")
        p.path = imageUri!!
        val data = ServicePeintreRealisationEntity(
